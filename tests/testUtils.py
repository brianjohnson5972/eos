...
class BlockData(object):
    ...
    def __init__(self, num, id, producer, time, received, lib, trxs, rcvOrProduced, previous):
        ...
        if previous is not None:
            ...
            self.libChanged = True
            self.producerSequence = 1


    def __str__(self):
        libChanged = "*" if self.libChanged else ""
        ...
        return "num: %s, id: %s, producer: %s (%d), time: %s, rcvd: %s, delay: %s sec, lib: %s%s, trxs: %s, slot: %s, dbr: %s sec, delta: %s, previous id: %s, %s" % \
               (self.num, self.id, self.producer, self.producerSequence, self.time.strftime(BlockData.TimeFmt), self.received.strftime(BlockData.TimeFmt),
                self.delay.total_seconds(), self.lib, libChanged, self.trxs, self.timePassing.total_seconds(), self.delayBetweenReceived.total_seconds(),
...
class NodeosLogBlockReader:
    ...
    def next(self):
        ...
        class ForkData:
            def __init__(self):
                self.accummulatedSlots = 0.0
                self.missedSlots = 0

            def add(self, block):
                slots = BlockData.numSlots(block.timePassing.total_seconds())
                self.accummulatedSlots += block.timePassing.total_seconds()
                self.missedSlots += slots - 1
        ...
        if printHeadForkAnalysis:
            ...
            if printHeadForkAnalysis.accummulatedSlots > 0.0:
                neitherForkSlots = BlockData.numSlots(current.timePassing.total_seconds() - BlockData.SlotTimeSec - printHeadForkAnalysis.accummulatedSlots)
                moreDesc = ""
                if neitherForkSlots < 0:
                    neitherForkSlots += printHeadForkAnalysis.missedSlots
                    moreDesc = "*** WHY NEGATIVE - ERROR? ****" if neitherForkSlots < 0 else ""
                self.forkAnalysisFile.write("Old Fork missed %d blocks, neither fork accounts for %d blocks%s.\n" % (printHeadForkAnalysis.missedSlots, neitherForkSlots, moreDesc))
