...
try:
    ...
    while True:
        ...
        if producerToSlot[lastBlockProducer]["count"]!=inRowCountPerProducer:
            Utils.errorExit("Producer %s, in slot %d, expected to produce %d blocks but produced %d blocks" % (blockProducer, producerToSlot[lastBlockProducer]["slot"], inRowCountPerProducer, producerToSlot[lastBlockProducer]["count"]))
