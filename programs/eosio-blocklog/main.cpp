...
trim_data::trim_data(bfs::path block_dir) {
   ...
   EOS_ASSERT( size == sizeof(version), block_log_unsupported_version, "invalid format for file ${file}", ("file",block_file_name.string()));
   ...
   if (version == 1)
      ...
      EOS_ASSERT(size == sizeof(first_block), block_log_exception, "invalid format for file ${file}",
                 ("file", block_file_name.string()));
   }
   cout << "first block= " << first_block << '\n';
   const uint64_t file_end = fseek(ind_in, 0, SEEK_END);                //get length of blocks.index (gives number of blocks)
...
void trim_data::find_block_pos(uint32_t n) {
   ...
   const uint64_t pos= fseek(ind_in, index_pos, SEEK_SET);
   ...
   EOS_ASSERT( size == sizeof(fpos0), block_log_exception, "cannot read blocks.index entry for block ${b}", ("b",n) );
   size = fread((void*)&fpos1,sizeof(fpos1), 1, ind_in);                   //filepos of block n+1
   EOS_ASSERT( size == sizeof(fpos1), block_log_exception, "cannot read blocks.index entry for block ${b}", ("b",n + 1) );
   ...
   const uint64_t block_offset_pos = fseek(blk_in, fpos0 + blknum_offset, SEEK_SET);
   ...
   EOS_ASSERT( size == sizeof(prior_blknum), block_log_exception, "cannot read prior block");
...
int trim_blocklog_front(bfs::path block_dir, uint32_t n) {        //n is first block to keep (remove prior blocks)
   ...
   *(uint32_t*)buf = 2;
   ...
   EOS_ASSERT( size == sizeof(uint32_t), block_log_exception, "blocks.out write fails" );
   size = fwrite((void*)&n, sizeof(n), 1, blk_out);                            //write first block number
   EOS_ASSERT( size == sizeof(n), block_log_exception, "blocks.log read fails" );

   const auto past_version_offset = td.version == 1 ? 4: 8;
   fseek(td.blk_in, past_version_offset, SEEK_SET) ;                //position past version number and maybe block number
   const auto header_len = 0x6e;
   size = fread((void*)buf, header_len, 1, td.blk_in);
   EOS_ASSERT( size == header_len, block_log_exception, "blocks.log read fails" );
   ...
   EOS_ASSERT( size == header_len + totem_len, block_log_exception, "blocks.out write fails" );
   ...
   uint64_t pos = fseek(td.blk_in, 0, SEEK_END);                     //get blocks.log file length
   uint32_t last_buf_len = (pos-td.fpos0 >= buf_len)? buf_len: pos-td.fpos0; //bytes to read from blk_in
   pos = fseek(td.blk_in, -(uint64_t)last_buf_len, SEEK_END);        //pos is where read last buf from blk_in
   ...
   for (;;) {        //invariant: at top of loop index_end and bnum are set and index_end is >= 0
      ...
      *(uint64_t *) (buf + index_end) = file_pos - pos_delta;     //update file position in buf
      ...
      if (bnum == blk_base) {                                     //if fpos_list is full write it to file
         ...
         fseek(ind_out, (blk_base - n) * sizeof(uint64_t), SEEK_SET);
         ...
         if (bnum == n) {                                         //if done with all blocks >= block n
            ...
            fseek(blk_out, pos-pos_delta, SEEK_SET);
            size = fwrite((void*)buf, last_buf_len, 1, blk_out);
            EOS_ASSERT( size == last_buf_len, block_log_exception, "blocks.out write fails" );
            break;
         } else {
            blk_base -= (buf_len>>3);
      ...
      if (index_end < 0) {                                        //the file pos record straddles buf boundary
         ...
         fseek(blk_out, pos-pos_delta + fk, SEEK_SET);
         size = fwrite((void*)(buf + fk), last_buf_len - fk, 1, blk_out);             //skip first 4K of buf write out the rest
         EOS_ASSERT( size == last_buf_len - fk, block_log_exception, "blocks.out write fails" );
...
int make_index(const bfs::path& block_dir, const bfs::path& out_file) {
   ...
   cout << "Reading version size " << size << " expecting " << sizeof(version) <<  '\n';
//   EOS_ASSERT( size == sizeof(version), block_log_exception, "${file} read fails", ("file", block_file_name.string()) );
   cout << "block log version= " << version << '\n';
   EOS_ASSERT( version == 1 || version == 2, block_log_unsupported_version, "block log version ${v} is not supported", ("v",version));
   if (version == 1)
      first_block = 1;
      ...
      cout << "Reading first_block size " << size << " expecting " << sizeof(first_block) <<  '\n';
//      EOS_ASSERT( size == sizeof(first_block), block_log_exception, "${file} read fails", ("file", block_file_name.string()) );
   }
   cout << "first block= " << first_block << '\n';

   uint64_t pos = fseek(fin,0,SEEK_END);                    //get blocks.log file length
   ...
   pos = fseek(fin, -(uint64_t)last_buf_len, SEEK_END);     //one time read last_buf_len
   uint64_t did_read = fread((void*)buf, last_buf_len, 1, fin);//read tail of blocks.log file into buf
   cout << "Reading tail size " << did_read << " expecting " << sizeof(first_block) <<  '\n';
   EOS_ASSERT( did_read == last_buf_len, block_log_exception, "blocks.log read fails" );
   ...
   uint64_t ind_pos = fseek(fout, ind_file_len - last_ind_buf_len, SEEK_SET);
...
//            cout << "block " << setw(10) << bnum << "    file_pos " << setw(14) << file_pos << '\n';  //last progress indicator
         ...
         did_read = fseek(fout, ind_pos, SEEK_SET);
...
void smoke_test(bfs::path block_dir) {
   ...
   fseek(td.blk_in, file_pos + blknum_offset, SEEK_SET);
   uint32_t bnum;
